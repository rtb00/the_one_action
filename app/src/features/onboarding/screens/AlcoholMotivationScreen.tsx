...
export default function AlcoholMotivationScreen() {
  const navigate = useNavigate();
  const { setAlcohol } = useOnboardingStore();
  const [motivation, setMotivation] = useState(5);

  const handleContinue = () => {
    setAlcohol(true, motivation);
    if (motivation === 0) {
      navigate('/onboarding/alcohol/info');
    } else {
      navigate('/onboarding/alcohol/reflection');
    }
  };
          ...
          <GlassCard className="space-y-8">
            <div className="text-center space-y-4">
              ...
              <h2 className="text-3xl font-semibold text-gray-900">
                Wie hoch ist deine Motivation, den Konsum zu reduzieren?
              </h2>
            </div>
